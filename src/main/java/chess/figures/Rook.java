...
public class Rook extends Figure {
    ...
    public boolean validMove(Position actualPos, Position targetPos, Board board) {
        ...
        if (actualPos.getPOS_X() != targetPos.getPOS_X() && actualPos.getPOS_Y() != targetPos.getPOS_Y()) {
            return false;
        }
    ...
    private int getDirection(Position actualPos, Position targetPos, boolean axis) {
        if (actualPos.getPOS_X() > targetPos.getPOS_X() && !axis) {
            return -1;
        }
        if (actualPos.getPOS_Y() > targetPos.getPOS_Y() && axis) {
            return -1;
        }
        return 1;
    }
